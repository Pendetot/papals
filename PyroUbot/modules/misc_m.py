    ...
    if status and hasattr(status, "text"):
        ...
        if pjg <= 100:
            if client.me.is_premium:
                text = f"""
<blockquote>{pong} Status : Premium {sks}
{tion} Reason : Akun anda tidak di batasi {sks}
{yubot} U-BOT : {bot.me.mention}</blockquote>
"""
            else:
                text = f"""
<blockquote><b>Status : Non-Premium
Reason : Akun anda aman dari limit
U-BOT : {bot.me.mention}</b></blockquote>
"""
            ...
            if client.me.is_premium:
                text = f"""
<blockquote><b>{pong} Status : Premium
{tion} Reason : Akun anda bermasalah{ggl} 
{yubot} U-BOT : {bot.me.mention}</b></blockquote>
"""
            else:
                text = f"""
<blockquote><b>>Status : Non-Premium
Reason : akun anda bermasalah
U-BOT : {bot.me.mention}</b><blockquote>
"""
            await client.send_message(message.chat.id, text)
            return await client.invoke(DeleteHistory(peer=bot_info, max_id=0, revoke=True))

    ...
    for my in ubot._ubot:
        if int(get_id[2]) == my.me.id:
            ...
            if my.me.id == OWNER_ID:
                status = " 𝚅𝚎𝚡𝚢𝙼𝚊𝚗-ᴜʙᴏᴛ  <code>[ᴏᴡɴᴇʀ]</code>"
            elif my.me.id in await get_list_from_vars(client.me.id, "SELER_USERS"):
                status = "𝚅𝚎𝚡𝚢𝙼𝚊𝚗-ᴜʙᴏᴛ ᴠ1  </b> <code>[ʀᴇsᴇʟʟᴇʀ]</code>"
            else:
                status = "𝚅𝚎𝚡𝚢𝙼𝚊𝚗-ᴜʙᴏᴛ ᴠ1  </b> <code>[ᴘʀᴇᴍɪᴜᴍ]</code>"
...
async def help_callback(client, callback_query):
    ...
    if mod_match:
        ...
        await callback_query.edit_message_text(
            text=text 
            + f"<blockquote><b>-- USERBOT 10K/BULAN BY @VexyMan_Ubot --</b></blockquote>\n<blockquote>• <b>-- CARA PENGGUNAAN TANYA @Vexy03 --</b></blockquote>",
           reply_markup=InlineKeyboardMarkup(button),
